...
use crate::lab3::{backend::BinStorageClient, frontend::FrontEnd, keeper};
...
pub async fn new_bin_client(backs: Vec<String>) -> TribResult<Box<dyn BinStorage>> {
    Ok(Box::new(
        BinStorageClient::new(
            backs
                .iter()
                .map(|back| format!("http://{}", back))
                .collect(),
        )
        .await?,
    ))
...
pub async fn new_front(
    ...
    Ok(Box::new(FrontEnd { bin_storage }))
}
